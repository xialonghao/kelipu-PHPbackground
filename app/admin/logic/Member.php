...
class Member extends AdminBase
{
    ...
    public function exportMemberList($where = [], $field = 'm.*,b.nickname as leader_nickname', $order = '')
    {
        ...
        $titles = "用户名,昵称,邮箱,手机,注册时间,上级";
        $keys   = "username,nickname,email,mobile,create_time,leader_nickname";
    ...
    public function addToGroup($data = [])
    {
        ...
        if (SYS_ADMINISTRATOR_ID == $data['id']) {
            ...
            return [RESULT_ERROR, '超级管理员不能被授权哦~', $url];
        }
    ...
    public function memberAdd($data = [])
    {
        ...
        {
         return [RESULT_ERROR,'请输入正确的用户名'];
        }

        if(trim($data['nickname'])  == '' )
        {
         return [RESULT_ERROR,'请输入正确的昵称'];
        }
        $row=db('member')->where('username = "'.$data['username'].'" and status != -1')->find();
        if($row)
        {
    ...
    public function memberEdit($data = [])
    {
        ...
        if(trim($data['username'])  == '')
        {
         ...
         if(strlen(trim($data['nickname']))> 16 )
        {
         return [RESULT_ERROR,'昵称最大长度16位'];
        }
        $row=db('member')->where('username = "'.$data['username'].'" and id != "'.$data['id'].'" and status != -1')->find();
        if($row)
        {
              return [RESULT_ERROR,'用户名已存在'];
        }
        ...
        $member = db('member')->where('id = "'.$data['id'].'"')->find();

       if(session('member_info')['id']!=1)
        {
            if( $member['leader_id'] !=session('member_info')['id']  )
            {
                  return [RESULT_ERROR, '无权限操作'];
            }
        }
     ...
     * 会员编辑
     */
    public function memberEdits($data = [])
    {
    ...
    public function editPassword($data = [])
    {
        ...
        if(session('member_info')['id']!=1 && $member['id']!= session('member_info')['id'] )
        {
        ...
        return $result ? [RESULT_SUCCESS, '密码修改成功', $url] : [RESULT_ERROR, '无操作或误操作'];
    }
    
    /**
     * 修改密码
     */
    public function editPassworde($data = [])
    {
        
        $validate_result = $this->validateMember->scene('passwords')->check($data);
        ...
        // if (data_md5_key($data['password']) != data_md5_key($data['password_confirm'])) {
    ...
    public function memberDel($where = [])
    {
        ...
        if (SYS_ADMINISTRATOR_ID == $where['id'] || MEMBER_ID == $where['id']) {
            ...
            return [RESULT_ERROR, '超级管理员和自己不能删除哦~', $url];
        }
        $member = db('member')->where('id = "'.$where['id'].'"')->find();
