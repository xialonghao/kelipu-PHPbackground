...
class AuthGroup extends AdminBase
{
    ...
    {
        $orw=db('auth_group')->where('name= "'.trim($data['name']).'" and status !=-1 and member_id = "'.MEMBER_ID.'"')->find();
        if ($orw) {
            return [RESULT_ERROR, '权限组名称不能重复'];
        }
        ...
        return $result ? [RESULT_SUCCESS, '权限组添加成功', $url] : [RESULT_ERROR, '无操作/误操作'];
    }
    ...
    public function groupEdit($data = []) 
    {
         ...
         $orw=db('auth_group')->where('name= "'.trim($data['name']).'" and status !=-1 and member_id = "'.MEMBER_ID.'" and id !="'.$data['id'].'"')->find();
        ...
        return $result ? [RESULT_SUCCESS, '权限组编辑成功', $url] : [RESULT_ERROR, '无操作/误操作'];
    }
    ...
    public function groupDel($where = [])
    {
        ...
        return $result ? [RESULT_SUCCESS, '权限组删除成功'] : [RESULT_ERROR, '无操作/误操作'];
    }
