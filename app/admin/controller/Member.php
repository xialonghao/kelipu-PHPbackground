...
class Member extends AdminBase
{
    ...
    public function memberEdits()
    {
        ...
        IS_POST && $this->jump($this->logicMember->memberEdits($this->param));
        

        $info = $this->logicMember->getMemberInfo(['id' => session('member_info')['id']]);
        
        $this->assign('info', $info);
        
        return $this->fetch('member_edits');
    }
     ...
     * 修改密码
     */
    public function editPassworde()
    {

        
        IS_POST && $this->jump($this->logicMember->editPassworde($this->param));
        
        $info = $this->logicMember->getMemberInfo(['id' => $this->param['id']]);
        
        $this->assign('info', $info);
        
        return $this->fetch('edit_passworde');
    }
}
