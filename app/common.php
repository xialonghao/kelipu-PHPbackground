...
define('KLP_API', 'http://uatopenapi.colipu.com');  //api地址
define('API_USERNAME', 'GiftCard');                 //用户名
define('API_PASSWORD', 'colipu_uat');               //密码
...
//获取access_token
 function get_access_tokens($username='',$password=''){   
...
//刷新access_token
 function get_refresh_tokens($refresh_token) {
...
//判断access_token有效期 , 过期后自动更新 夏龙浩版
function judgeValidityst() {

    $token         = Cache::get('thoken');
    $time          = time();
    if($time > strtotime($token['refresh_expires_at']))
    {
        $arr=['code'=>200,$time,strtotime($token['refresh_expires_at'])];
        return $arr;               
    }
    else
    {
...
//判断access_token有效期 , 过期后自动更新
function judgeValiditys() {
        //获取到正确的access_token
        if(!empty(Cache::get('get_access_token_goods')))
        {
            ...
            if($time > strtotime($result['expires_at']))
            {
                ...
                //刷新token
                $row=get_refresh_tokens($refresh_token);  
            }
            
            $result = session('get_access_token_goods');
        }else 
        {   
            $status = get_access_tokens(API_USERNAME,API_PASSWORD);
            if($status !== true)
            {
                return false;
            }
...
function juhecurl_gets($url,$headers,$params=false,$ispost=0){
        // $headers = array(
        //      "Colipu-Token:".$header,      
        //  );
        $ch = curl_init();
...
function show($code,$data='',$msg='success')
{
  $arr=array(
    'code'=>$code,
    'msg'=>$msg,
...
{
    $row=db('goods_category')->where('id ="'.$id.'"')->find();
    return $row['cate_name'];  
}

function cateaffiliation($project_id)
{
